...
Assert-Contains $header "#define\s+OTA_MANIFEST_SIZE\s+40U" "manifest size must be fixed at 40 bytes"
...
Assert-Contains $header "(?s)typedef\s+struct\s*\{\s*uint32_t\s+magic;\s*uint16_t\s+version;\s*uint16_t\s+header_size;\s*uint32_t\s+state;\s*uint32_t\s+image_size;\s*uint32_t\s+image_crc32;\s*uint32_t\s+received_size;\s*uint32_t\s+target_app_addr;\s*uint32_t\s+target_app_max_size;\s*uint32_t\s+sequence;\s*uint32_t\s+manifest_crc32;\s*\}\s+ota_manifest_t;" "manifest fields must remain complete and ordered"
