...
{
	int ret=HAL_I2C_Mem_Write(&hi2c2, slave_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, data, length, 0XFFFF);
...
{
	int ret= HAL_I2C_Mem_Read(&hi2c2, slave_addr, reg_addr, I2C_MEMADD_SIZE_8BIT, data, length, 0XFFFF);
	if(ret==HAL_OK)return 0;
	else return -1;
}
/* USER CODE END 1 */
